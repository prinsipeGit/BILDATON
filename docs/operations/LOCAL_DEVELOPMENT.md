...
PostgreSQL is hosted by Supabase. The API uses Supabase itself as the durable message inbox, so Redis is not required for the chatbot MVP.
...
4. Configure Meta and OpenAI credentials in the ignored local `.env` file.
5. Run `npm install` and `npm run db:generate`.
6. Apply committed migrations to Supabase with `npm run db:deploy`, then run `npm run db:seed`. Use `npm run db:migrate` only while intentionally creating a new development migration.
7. Run the API with `node --env-file=.env --import tsx apps/api/src/server.ts`.
...
`GET /ready` checks PostgreSQL. It returns `200 READY` only when the database responds, otherwise `503 NOT_READY`. Dependency errors are not returned to clients.
